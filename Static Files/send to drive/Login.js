    ...
    fetch("http://localhost:8081/login", {
      ...
      .then(responseJsonEmailExist => {
        if (responseJsonEmailExist.length > 0) {
          ...
          localStorage.setItem("_id", responseJsonEmailExist[0]._id);
          localStorage.setItem("userName", responseJsonEmailExist[0].userName);
          this.setState({
            ...
            .then(EmailpassMatch => {
              if (EmailpassMatch.length > 0) {
                ...
                if (EmailpassMatch[0].verifyStatus === true) {
                  ...
                  this.setState({
                    verifyAlert: "pls go and verify your email"
                  });
                }
              } else {
