...
router.post("/submitData", async function(req, res) {
  try {
    ...
    var mailoptions = {
      ...
      html:
        '<p>Click <a href="http://localhost:8081/verifyonSignup/' +
        req.body.email +
        '">here mr.</a> to reset your password</p>' // send only id for further use of this id
    };
    var mailoption = transporter.sendMail(mailoptions, function(error, info) {
      if (error) {
        console.log("mail send error", error);
        // res.send({ mailsendError: error });
        // res.send(signUp);
      } else {
        console.log("signup respons", signUp);
...
router.post("/resetPass", async function(req, res) {
  try {
    ...
    resetPass.n > 0
      ? res.send(resetPass)
      : res.send({ msg: "your pass is not reset" });
  } catch (err) {
    console.log(err);
  }
});
